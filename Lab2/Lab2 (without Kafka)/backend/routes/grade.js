...
router.get("/", function(req, res) {
  console.log("Get an assignment/quiz grade data called!", req.query);
        ...
        if (user) {
          ...
          if (user["grades"] != undefined) {
            if (user["grades"][gradeData.courseId] != undefined) {
              if (user["grades"][gradeData.courseId][gradeData.assignmentId] != undefined) {
                res.status(200).send(user["grades"][gradeData.courseId][gradeData.assignmentId]);
              } else {
                res.status(400).send();
              }
            } else {
    ...
    Model.userDetails.findOne(
      ...
      (err, user) => {
        if (err) {
          ...
          // FIXME user["grades"] is getting overriden when we have submissions of 2 assignments simultaneosly
