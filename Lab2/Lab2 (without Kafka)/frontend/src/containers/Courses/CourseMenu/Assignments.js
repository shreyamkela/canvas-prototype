...
class Assignment extends Component {
  ...
  handleSubmit = async (id, dueBy) => {
    ...
    if (dueBy >= todayDate) {
      try {
        let results = await S3FileUpload.uploadFile(this.state.selectedFile, config);
        const { loginRequest, currentCourseDataToComponent } = this.props;
        const data = {
          email: loginRequest.email,
          persona: loginRequest.persona,
          courseId: currentCourseDataToComponent.currentCourse.courseId,
          document: { file: `${results.location}`, assignmentId: id }
        };
        let response = await API.post("assignment", { data });
        message.success("Uploaded successfully!");
        // Reload after 2 sec so that success message is shown and then the chosen file name does not show beside choose file button
        setTimeout(2000);
  ...
  handleViewDocument = async key => {
            ...
            <form onSubmit={this.handleGradeSubmit}>
              ...
              <table>
                <tr>
                  ...
                  <th>{maxGrade}</th>
                  <th>
                    <button className="btn btn-success btn-sm mx-4">Submit</button>
                  </th>
                </tr>
              </table>
            </form>
    ...
    // FIXME Add functionality for the student to view "Grade of this assignment" when they open the document
      ...
      let currentGrade = "Grade not assigned yet";
      try {
      ...
      grade = (
        <React.Fragment>
          Grade of this assignment: {currentGrade}
          <br />
        </React.Fragment>
      );
    }
    ...
    if (loginRequest.persona == "1") {
      documentLink = this.state.viewSubmissions[key][this.state.viewSubmissions[key].length - 1];
    } else if (loginRequest.persona == "2") {
      documentLink = this.state.viewSubmissions[key];
    }
    let viewDocument = (
      <React.Fragment>
        <div>
            ...
            <tr>
              ...
              <th>
                <div style={{ marginLeft: 540 }}>
                  <button className="btn btn-primary btn-sm mx-4" onClick={this.reloadPage}>
                    Back to Assignments
                  </button>
                </div>
              </th>
        ...
        <iframe title="file" style={{ width: "100%", height: 500 }} src={documentLink} />
      </React.Fragment>
    );
            ...
            .map(key => (
              <div>
                ...
                <div style={{ marginLeft: 40 }}>
                  ...
                  >
                    {/* this.state.viewSubmissions[key].split("/") gives an array containing the parts of the URL of the pdf, split at / */}
                    {
                      this.state.viewSubmissions[key][this.state.viewSubmissions[key].length - 1].split("/")[
                        this.state.viewSubmissions[key][this.state.viewSubmissions[key].length - 1].split("/").length - 1
                      ]
                    }
                  </Link>
                </div>
            ...
            .map(key => (
              <div>
                ...
                >
                  ...
                  {/* this.state.viewSubmissions[key].split("/") gives an array containing the parts of the URL of the pdf, split at / */}
                  {typeof this.state.viewSubmissions[key] == "string"
                    ? this.state.viewSubmissions[key].split("/")[this.state.viewSubmissions[key].split("/").length - 1]
                    : null}
                </Link>
                <br />
              </div>
