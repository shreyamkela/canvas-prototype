    ...
    (err, user) => {
      if (err) {
        ...
        if (user) {
          ...
          res.status(400).end("Email already registered!"); // Bad request - Catch this error at frontend axios
        } else {
        ...
        user.save().then(
          doc => {
            ...
            res.status(200).end("Registration Successful!"); // status should come before send
          },
          err => {
